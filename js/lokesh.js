...
( () => {
    ...
    function typewriter() {
        ...
        if (sub) {
            const words = (sub.dataset.texts || '').split(',').map(w => w.trim()).filter(Boolean);
            if (!words.length)
                return;
            let wi = 0
              , chars = 0
            ...
            const loop = t => {
                ...
                sub.textContent = w.slice(0, Math.floor(Math.max(0, Math.min(w.length, chars))));
                raf(loop);
            }
            ;
            raf(loop);
            ...
            const io = new IntersectionObserver(es => {
                es.forEach(en => {
                    ...
                    links.forEach(l => {
                        ...
                        match ? l.setAttribute('aria-current', 'true') : l.removeAttribute('aria-current');
                    }
                    );
                }
                );
    ...
    function parallax() {
        ...
        hero.addEventListener('pointerleave', () => particles.forEach(p => p.style.transform = ''));
        let scTick = false;
        const onScroll = () => {
            if (scTick)
                return;
    ...
    function horizontalScroll() {
        const defs = [['.skills.skills--hscroll', '.skills.skills--hscroll .skills-grid-min', '--hscroll-height'], ['#achievements.achievements--hscroll', '#achievements.achievements--hscroll .achievements-grid', '--ach-h']];
        ...
        const setup = (wrapSel, rowSel, cssVar) => {
            ...
            function layout() {
                ...
                wrap.style.setProperty(cssVar, `${Math.max(travel, window.innerHeight - navH() + 200)}px`);
            ...
            function onScroll() {
                ...
                raf( () => {
                    ...
                    if (r.top <= start && r.bottom > end) {
                        const prog = Math.min(1, Math.max(0, (start - r.top) / Math.max(1, total)));
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        navigation();
        contact();
        (window.requestIdleCallback ? requestIdleCallback : fn => setTimeout(fn, 50))( () => {
            parallax();
            horizontalScroll();
            rippleDelegated();
        }
